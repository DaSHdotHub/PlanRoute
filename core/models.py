...
class Distance(models.Model):
    from_address = models.ForeignKey(
        Address, on_delete=models.CASCADE, related_name="distance_from"
    )
    to_address = models.ForeignKey(
        Address, on_delete=models.CASCADE, related_name="distance_to"
    )
    distance_in_km = models.FloatField(
        null=True, blank=True, help_text="Distance in kilometers"
    )
    last_updated = models.DateTimeField(null=True, blank=True, auto_now=True)
    travel_mode = models.CharField(
        max_length=10, default="car", help_text="Mode of travel"
    )
